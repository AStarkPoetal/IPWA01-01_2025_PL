document.addEventListener("DOMContentLoaded", () => {
  ...
  // ---- SUCHE (4 FELDER, KOMBINIERTE FILTERUNG ----
  ...
  function filterRows() {
    ...
    const qHeaderCompany = clean(headerCompanyInput?.value).trim().toLowerCase();
    ...
    tbody.querySelectorAll("tr").forEach(row => {
      ...
      const ok =
        ...
        (!qHeaderCompany || company.includes(qHeaderCompany));

      row.style.display = ok ? "" : "none";
    });
  }

  [companyInput, installationInput, countryInput, emissionsInput, headerCompanyInput].forEach(inp => {
    inp?.addEventListener("input", filterRows);
  });

  // ---- SORTIEREN MIT PFEILEN ----
